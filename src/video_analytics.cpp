...
void HogAnalytics::VideoNalyse() {
	...
	while (ifPlay) {
		if (!ifPuase) {
			...
			cuda::cvtColor(g_frame, gray, COLOR_BGR2GRAY);
			hog->detectMultiScale(gray, objects);

			for (int i = 0; i < objects.size(); i++) {
				rectangle(frame, objects[i], Scalar(0, 0, 255), 2, 8, 0);
			}
...
void ObjDetect_yolov5::VideoObjDetect()
{

	bool ifPuase = false;
	bool ifPlay = true;

	namedWindow("yolov5 detect");
...
//			std::vector<float> confidences;
//
//			const double score_threshould = 0.5;
...
//				//cv::minMaxLoc(classes_scores, 0, &score, 0, &classIdPoint);
//				// 置信度 0～1之间
//				if (score > score_threshould)
//				{
//					float* classes_scores = data + 5;
//					cv::Mat scores(1, label_obj_.classes_names_.size(), CV_32FC1, classes_scores);
...
//			// NMS
//			std::vector<int> indexes;
//			cv::dnn::NMSBoxes(boxes, confidences, score_threshould, 0.5, indexes);
//			for (size_t i = 0; i < indexes.size(); i++) {
//				int index = indexes[i];
//				int idx = classIds[index];
//				cv::rectangle(frame, boxes[index], colors[idx % 5], 2, 8);
